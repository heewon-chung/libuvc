...
Implementation of the R1CS VC (Verifiable Computation) scheme.

This is a Groth16 variant without zero-knowledge randomization (no r,s).
See r1cs_vc_ppzksnark.hpp for details.
...
#ifndef R1CS_VC_PPZKSNARK_TCC_
#define R1CS_VC_PPZKSNARK_TCC_
...
bool r1cs_vc_ppzksnark_proving_key<ppT>::operator==(const r1cs_vc_ppzksnark_proving_key<ppT> &other) const
{
...
{
    in >> pk.alpha_g1; libff::consume_OUTPUT_NEWLINE(in);
    in >> pk.beta_g1;  libff::consume_OUTPUT_NEWLINE(in);
    in >> pk.beta_g2;  libff::consume_OUTPUT_NEWLINE(in);
    in >> pk.delta_g2; libff::consume_OUTPUT_NEWLINE(in);
    in >> pk.A_query;
    in >> pk.B_query;
    in >> pk.H_query;
    in >> pk.L_query;
...
template<typename ppT>
bool r1cs_vc_ppzksnark_verification_key<ppT>::operator==(const r1cs_vc_ppzksnark_verification_key<ppT> &other) const
{
...
{
    in >> vk.alpha_g1_beta_g2; libff::consume_OUTPUT_NEWLINE(in);
    in >> vk.gamma_g2;         libff::consume_OUTPUT_NEWLINE(in);
    in >> vk.delta_g2;         libff::consume_OUTPUT_NEWLINE(in);
    in >> vk.gamma_ABC_g1;     libff::consume_OUTPUT_NEWLINE(in);
    return in;
}
...
template<typename ppT>
bool r1cs_vc_ppzksnark_processed_verification_key<ppT>::operator==(const r1cs_vc_ppzksnark_processed_verification_key<ppT> &other) const
{
...
template<typename ppT>
bool r1cs_vc_ppzksnark_proof<ppT>::operator==(const r1cs_vc_ppzksnark_proof<ppT> &other) const
{
...
{
    in >> proof.g_A; libff::consume_OUTPUT_NEWLINE(in);
    in >> proof.g_B; libff::consume_OUTPUT_NEWLINE(in);
    in >> proof.g_C; libff::consume_OUTPUT_NEWLINE(in);
    return in;
}
...
/**
 * VC.Setup (Generator/Key Generation)
 *
 * Given a R1CS constraint system, produces proving and verification keys.
 * Follows Paper Section 4.1:
 *   1. Sample random alpha, beta, gamma, delta, x <- F*
 *   2. QAP reduction at evaluation point x
 *   3. Encode polynomials into group elements
 *
 * Identical to Groth16 generator — the difference is only in the prover.
 */
template <typename ppT>
r1cs_vc_ppzksnark_keypair<ppT> r1cs_vc_ppzksnark_generator(const r1cs_vc_ppzksnark_constraint_system<ppT> &r1cs)
{
    libff::enter_block("Call to r1cs_vc_ppzksnark_generator");

    /* Make the B_query "lighter" if possible */
    r1cs_vc_ppzksnark_constraint_system<ppT> r1cs_copy(r1cs);
    ...
    {
        if (!qap.At[i].is_zero()) ++non_zero_At;
        if (!qap.Bt[i].is_zero()) ++non_zero_Bt;
    }
    ...
    /* gamma_ABC: (beta*A_i(t) + alpha*B_i(t) + C_i(t)) * gamma^{-1} for statement indices */
    libff::enter_block("Compute gamma_ABC for VC verification key");
    ...
    libff::leave_block("Compute gamma_ABC for VC verification key");

    /* L_query: (beta*A_i(t) + alpha*B_i(t) + C_i(t)) * delta^{-1} for witness indices */
    libff::enter_block("Compute L query for VC proving key");
    ...
    libff::leave_block("Compute L query for VC proving key");

    /* H for Groth16/VC is degree d-2 */
    Ht.resize(Ht.size() - 2);
    ...
    libff::enter_block("Generate VC proving key");
    ...
    libff::leave_block("Generate VC proving key");

    libff::enter_block("Generate VC verification key");
    libff::GT<ppT> alpha_g1_beta_g2 = ppT::reduced_pairing(alpha_g1, beta_g2);
    libff::G2<ppT> gamma_g2 = gamma * G2_gen;

    libff::enter_block("Encode gamma_ABC for VC verification key");
    ...
    libff::leave_block("Encode gamma_ABC for VC verification key");
    libff::leave_block("Generate VC verification key");

    libff::leave_block("Call to r1cs_vc_ppzksnark_generator");

    accumulation_vector<libff::G1<ppT> > gamma_ABC_g1(std::move(gamma_ABC_g1_0), std::move(gamma_ABC_g1_values));

    r1cs_vc_ppzksnark_verification_key<ppT> vk = r1cs_vc_ppzksnark_verification_key<ppT>(
        alpha_g1_beta_g2, gamma_g2, delta_g2, gamma_ABC_g1);

    r1cs_vc_ppzksnark_proving_key<ppT> pk = r1cs_vc_ppzksnark_proving_key<ppT>(
        std::move(alpha_g1),
        std::move(beta_g1),
        std::move(beta_g2),
        std::move(delta_g2),
    ...
    return r1cs_vc_ppzksnark_keypair<ppT>(std::move(pk), std::move(vk));
}
 ...
 * VC.Prove (Prover)
 *
 * Computes a deterministic proof without ZK randomization.
 * Follows Paper Section 4.1:
 *   A = alpha + sum_i(a_i * A_i(t))
 *   B = beta  + sum_i(a_i * B_i(t))
 *   C = sum_{i in I^wt}(a_i * L_i(t)) + H(t)*Z(t)/delta
 *
 * Key difference from Groth16: NO r,s random elements, NO r*delta or s*delta terms.
 */
template <typename ppT>
...
{
    libff::enter_block("Call to r1cs_vc_ppzksnark_prover");
    ...
    /* d1=d2=d3=0: no ZK randomization in QAP witness */
    const qap_witness<libff::Fr<ppT> > qap_wit = r1cs_to_qap_witness_map(
        pk.constraint_system, primary_input, auxiliary_input,
    ...
    /* Prepare assignment vector: (1, a_1, ..., a_m) */
    ...
    /* sum_{i in I^wt} a_i * [(beta*A_i(t) + alpha*B_i(t) + C_i(t)) / delta]_1 */
    ...
    /**
     * Compute proof elements WITHOUT randomization (no r, s).
     *
     * A = alpha + sum_i(a_i * A_i(t))
     * B = beta  + sum_i(a_i * B_i(t))       [in G2]
     * C = sum_{i in I^wt}(a_i * L_i(t)) + H(t)*Z(t)/delta
     *
     * Compare with Groth16 which adds:
     *   A += r * delta
     *   B += s * delta
     *   C += s * A + r * B - r*s*delta
     */
    libff::G1<ppT> g1_A = pk.alpha_g1 + evaluation_At;
    libff::G2<ppT> g2_B = pk.beta_g2 + evaluation_Bt.g;
    libff::G1<ppT> g1_C = evaluation_Ht + evaluation_Lt;

    libff::leave_block("Compute the proof");

    libff::leave_block("Call to r1cs_vc_ppzksnark_prover");

    r1cs_vc_ppzksnark_proof<ppT> proof = r1cs_vc_ppzksnark_proof<ppT>(
...
/**
 * Process verification key for online verification.
 */
template <typename ppT>
r1cs_vc_ppzksnark_processed_verification_key<ppT> r1cs_vc_ppzksnark_verifier_process_vk(
    const r1cs_vc_ppzksnark_verification_key<ppT> &vk)
{
    libff::enter_block("Call to r1cs_vc_ppzksnark_verifier_process_vk");

    r1cs_vc_ppzksnark_processed_verification_key<ppT> pvk;
    ...
    libff::leave_block("Call to r1cs_vc_ppzksnark_verifier_process_vk");
...
/**
 * VC.Verify (Online Verifier, weak IC)
 *
 * Checks the pairing equation:
 *   e(A, B) = e(alpha, beta) * e(acc, gamma) * e(C, delta)
 *
 * where acc = gamma_ABC_g1[0] + sum_i a_i * gamma_ABC_g1[i]
 *
 * Identical verification equation to Groth16.
 */
template <typename ppT>
bool r1cs_vc_ppzksnark_online_verifier_weak_IC(
    const r1cs_vc_ppzksnark_processed_verification_key<ppT> &pvk,
    const r1cs_vc_ppzksnark_primary_input<ppT> &primary_input,
    const r1cs_vc_ppzksnark_proof<ppT> &proof)
{
    libff::enter_block("Call to r1cs_vc_ppzksnark_online_verifier_weak_IC");
    ...
    const accumulation_vector<libff::G1<ppT> > accumulated_IC =
        pvk.gamma_ABC_g1.template accumulate_chunk<libff::Fr<ppT> >(
    ...
    /* Check: e(A, B) * e(-acc, gamma) * e(-C, delta) == e(alpha, beta) */
    ...
    libff::leave_block("Call to r1cs_vc_ppzksnark_online_verifier_weak_IC");
...
bool r1cs_vc_ppzksnark_verifier_weak_IC(const r1cs_vc_ppzksnark_verification_key<ppT> &vk,
                                        ...
                                        const r1cs_vc_ppzksnark_proof<ppT> &proof)
{
    libff::enter_block("Call to r1cs_vc_ppzksnark_verifier_weak_IC");
    r1cs_vc_ppzksnark_processed_verification_key<ppT> pvk = r1cs_vc_ppzksnark_verifier_process_vk<ppT>(vk);
    bool result = r1cs_vc_ppzksnark_online_verifier_weak_IC<ppT>(pvk, primary_input, proof);
    libff::leave_block("Call to r1cs_vc_ppzksnark_verifier_weak_IC");
    return result;
}


template<typename ppT>
bool r1cs_vc_ppzksnark_online_verifier_strong_IC(
    const r1cs_vc_ppzksnark_processed_verification_key<ppT> &pvk,
    const r1cs_vc_ppzksnark_primary_input<ppT> &primary_input,
    const r1cs_vc_ppzksnark_proof<ppT> &proof)
{
    bool result = true;
    libff::enter_block("Call to r1cs_vc_ppzksnark_online_verifier_strong_IC");
    ...
    {
        result = r1cs_vc_ppzksnark_online_verifier_weak_IC(pvk, primary_input, proof);
    }

    libff::leave_block("Call to r1cs_vc_ppzksnark_online_verifier_strong_IC");
    return result;
}
...
bool r1cs_vc_ppzksnark_verifier_strong_IC(const r1cs_vc_ppzksnark_verification_key<ppT> &vk,
                                          ...
                                          const r1cs_vc_ppzksnark_proof<ppT> &proof)
{
    libff::enter_block("Call to r1cs_vc_ppzksnark_verifier_strong_IC");
    r1cs_vc_ppzksnark_processed_verification_key<ppT> pvk = r1cs_vc_ppzksnark_verifier_process_vk<ppT>(vk);
    bool result = r1cs_vc_ppzksnark_online_verifier_strong_IC<ppT>(pvk, primary_input, proof);
    libff::leave_block("Call to r1cs_vc_ppzksnark_verifier_strong_IC");
    return result;
}


} // libsnark
#endif // R1CS_VC_PPZKSNARK_TCC_
